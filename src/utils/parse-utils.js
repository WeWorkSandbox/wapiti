...
import { walkServices } from "./grpc";
import { mockRequestMethod, mockResponseMethod } from "./grpcMocks";

export async function processGrpcSpec(grpcSpec) {
  ...
  walkServices(root, (service, serviceName) => {
    ...
    tags.push(tagObj);
    Object.keys(service.methods).forEach( path => {
        totalPathCount++;
        tagObj.paths.push({
          "show"        : true,
          "method"      : 'post',
          ...
          "path"        : path,
          "parameters"  : [],
          "requestBody" : mockRequestMethod(service,service.methods[path]),
          "responses"   : [{status: 0, resp: mockResponseMethod(service,service.methods[path])}],
          "deprecated"  : false,
          "security"    : '',
        });
...
export async function ProcessSpec(specUrl){
